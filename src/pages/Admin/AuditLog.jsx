...
const mockAuditData = [
    ...
    { id: 2, user: 'asdsad', action: 'Started charging session', timestamp: '2024-06-01 10:05:00' },
...
const getActionIcon = (action) => {
    switch (action.toLowerCase()) {
        ...
        case 'started charging session':
            return <span className="text-xl text-green-500">⚡️</span>;
            ...
            <div className="w-full max-w-6xl p-6 sm:p-8 lg:p-12 ml-14 md:ml-64">
                ...
                <div className="mb-8 text-center">
                    <h1 className="text-3xl sm:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-800 to-green-700 animate-fade-in-down">
                        EV Station Audit Log
                    </h1>
                    <div className="flex flex-col sm:flex-row items-center justify-between mt-2">
                        <p className="text-gray-600 text-lg">Track all station activities and system events</p>
                        <div className="bg-green-100/50 px-4 py-2 rounded-full mt-2 sm:mt-0">
                            <span className="text-green-700 font-medium">{logs.length} Total Events</span>
                        </div>
                    </div>
                </div>

                {/* Audit Log Table */}
                <div className="bg-white rounded-3xl shadow-2xl border border-green-100/50 overflow-hidden animate-slide-in-up">
                    {/* Table Header */}
                    <div className="bg-gradient-to-r from-green-800 to-green-700 px-6 sm:px-8 py-5">
                    ...
                    <div className="max-h-[65vh] overflow-y-auto">
                        {logs.length === 0 ? (
                            ...
                            logs.map((log, index) => {
                                ...
                                return (
                                    <div
                                        className="grid grid-cols-[2fr_3fr_2fr_1fr] gap-6 items-center text-gray-600 py-4 px-6 sm:px-8 border-b border-green-100 hover:bg-green-50/50 transition-all duration-300 group"
                                        key={log.id}
                                        style={{
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-gray-800 group-hover:text-green-700 transition-colors duration-300">
                                                    {log.user}
                                                </p>
                    ...
                    {/* Footer with summary */}
                    {logs.length > 0 && (
                        <div className="bg-green-50/30 px-6 sm:px-8 py-4 border-t border-green-100">
                            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-center">
                                <div className="bg-white p-3 rounded-lg shadow-sm">
                                    <p className="text-lg font-bold text-green-700">
                                        {logs.filter(log => log.action.toLowerCase().includes('login')).length}
                                    </p>
                                    <p className="text-green-800 text-xs">Login Events</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg shadow-sm">
                                    <p className="text-lg font-bold text-green-700">
                                        {logs.filter(log => log.action.toLowerCase().includes('charging')).length}
                                    </p>
                                    <p className="text-green-800 text-xs">Charging Sessions</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg shadow-sm">
                                    <p className="text-lg font-bold text-green-700">
                                        {logs.filter(log => log.action.toLowerCase().includes('payment')).length}
                                    </p>
                                    <p className="text-green-800 text-xs">Payments</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg shadow-sm">
                                    <p className="text-lg font-bold text-green-700">
                                        {logs.length}
                                    </p>
                                    <p className="text-green-800 text-xs">Total Events</p>
                                </div>
                            </div>
                        </div>
                    )}
